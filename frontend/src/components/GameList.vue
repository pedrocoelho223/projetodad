...
const getPhotoUrl = (filename) => {
  ...
  // 1. Obtém o domínio da API (que termina em /api)
  let baseUrl = import.meta.env.VITE_API_DOMAIN || 'http://localhost:8000';

  // 2. Remove o "/api" do final para ficar com a raiz do servidor
  // Ex: http://api-dad.../api  --->  http://api-dad...
  baseUrl = baseUrl.replace(/\/api$/, '');

  // 3. Retorna o caminho completo para a pasta pública
