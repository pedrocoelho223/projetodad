      ...
      <table class="styled-table">
        <thead>
          <tr>
            <th>Data</th>
            <th>Tipo</th>
            <th>Jogadores</th> <th class="text-center">Placar</th> <th>Vencedor</th>
            <th>Ações</th>
          </tr>
        </thead>
        <tbody>
            ...
            <td>
              <div class="players-grid">
                <div class="player-row">
                    <span v-if="game.player1" class="player-name">
                        {{ game.player1.name }}
                    </span>
                ...
                <div class="player-row">
                    <span v-if="game.player2" class="player-name">
                        {{ game.player2.name }}
                    </span>
            ...
            <td>
              <div v-if="game.winner" class="winner-info">
                <img
                  ...
                  :src="`/storage/photos/${game.winner.photo_avatar_filename}`"
                  class="avatar"
...
const games = ref([])
const meta = ref({}) // Para guardar a paginação
...
const getTypeName = (type) => {
  // Ajusta estes valores conforme a tua lógica de negócio
...
const fetchGames = async (page = 1) => {
  loading.value = true
  // Atualiza a página na store antes de pedir
  api.gameQueryParameters.page = page

  try {
    const response = await api.getGames()
    games.value = response.data.data // A lista de jogos
    meta.value = response.data.meta // Os dados da paginação
  } catch (e) {
...
const createGame = async () => {
  ...
  try {
    ...
    // Volta à página 1 para ver o jogo novo
    await fetchGames(1)
  } catch (e) {
    console.error(e)
...
const joinGame = (gameId) => {
  ...
  // Aqui virá a lógica de websocket
}
