...
onMounted(async () => {
  ...
  // Recarregar dados para garantir que temos o saldo e info atualizada
...
<template>
  <div class="max-w-4xl mx-auto py-10 px-4">
    <div
      v-if="message.text"
      class="mb-4 p-4 rounded whitespace-pre-line"
      ...
      <div class="md:col-span-1">
        <div class="bg-white shadow rounded-lg p-6 text-center">
          <div class="relative w-32 h-32 mx-auto mb-4">
            <img
              :src="authStore.userPhotoUrl || '/anonymous.png'"
              alt="Avatar"
      ...
      <div class="mt-5 md:col-span-2 md:mt-0">
        <div class="bg-white shadow rounded-lg p-6">
          <div class="flex justify-between items-center mb-6 border-b pb-4">
            ...
            <button
              @click="isEditing = !isEditing; resetForm()"
          ...
          <form v-if="isEditing" @submit.prevent="handleUpdate" class="space-y-4">
            <div class="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <input v-model="form.nickname" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2">
              </div>
              ...
              <div>
                ...
                <input v-model="form.password" type="password" placeholder="Deixe vazio para manter" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2">
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-700">Alterar Foto</label>
                <input @change="onFileChange" type="file" accept="image/*" class="mt-1 block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"/>
              </div>
            </div>
          ...
          <div v-else class="space-y-4">
            ...
            <div class="border-t pt-6 mt-6">
              <button @click="showDeleteModal = true" class="text-red-600 hover:text-red-800 text-sm font-medium flex items-center gap-1">
      ...
      <div class="bg-white rounded-lg px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-w-lg w-full">
        <div class="sm:flex sm:items-start">
          ...
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
            ...
            <div class="mt-2">
              <p class="text-sm text-gray-500">Tem a certeza? Todos os seus dados, moedas e histórico de jogos serão perdidos.</p>
              <input v-model="deletePassword" type="password" placeholder="Confirme a sua password" class="mt-4 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 border p-2" />
            </div>
          </div>
        </div>
        <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
          <button @click="handleDeleteAccount" type="button" class="inline-flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 sm:ml-3 sm:w-auto sm:text-sm">Apagar</button>
          <button @click="showDeleteModal = false" type="button" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Cancelar</button>
        </div>
      </div>
    </div>
  </div>
